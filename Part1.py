...
################################# Rest for The Report  ##############################
"""
damali = cv2.imread("satranc.jpg", cv2.IMREAD_GRAYSCALE)

vertical = matrix_convolve(damali, sobel_x)
horizontal = matrix_convolve(damali, sobel_y)

normalized_vertical = np.uint8((vertical + 255)/2)
normalized_horizontal = np.uint8((horizontal + 255)/2)
cv2.imshow("norm_vertical", normalized_vertical)
cv2.imwrite("Report/normalized_vertical.png", normalized_vertical)
cv2.imshow("norm_horizontal", normalized_horizontal)
cv2.imwrite("Report/normalized_horizontal.png", normalized_horizontal)

edges = np.uint8(np.sqrt(np.square(vertical) + np.square(horizontal)))
cv2.imshow("Image", damali)
cv2.imshow("Edges", edges)
cv2.imwrite("Report/Edges.png", edges)
