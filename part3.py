...
corner = detect_corner(smooth_gray, 0.05) # k between 0.04 - 0.06
points = (corner > 0)
cropped[points] = [0, 255, 0]

cv2.imshow("image", cropped)
cv2.imwrite("harris_ss.png", cropped)
cv2.waitKey()

############################### Rest for the report #############################
"""
shapes = cv2.imread("koseler2.png")
gray_shapes = cv2.cvtColor(shapes, cv2.COLOR_BGR2GRAY)
smooth_shapes = matrix_convolve(gray_shapes, gaussian_five)

corners = detect_corner(smooth_shapes, 0.05)
point = (corners > 0)
shapes[point] = [0, 0, 255]

cv2.imshow("koseler", shapes)
cv2.imwrite("corners.png", shapes)
cv2.waitKey()
"""
